...
module.exports.destroyRelay = function($happn, key, callback) {

  var componentName = key.name;
  var code = key.code;

  if (relays[componentName].code != code) {
  ...
  if (targetEventApi = relays[componentName].targetEventApi) {
    if (subscriptions = relays[componentName].subscriptions) {
...
module.exports.createRelayConnected = function($happn, connection, relaySpec, callback) {
  ...
  return module.exports.createDynamicModule('connected', $happn, connection, relaySpec, function(e, instance, routes) {
    ...
    $happn._mesh._createElement(elementSpec, true, function(e) {
      if (e) {
        relays[componentName].subscriptions.forEach(function(refId) {
          // TODO: what happens in happn if it restarts, are these
          //       subscription id's still valid after reconnect??
          relays[componentName].targetEventApi.off(refId, function() {});
        });
        delete relays[componentName];
        return callback(e);
      }
      ...
      if (env = 'test') {
        relays[componentName] = relays[componentName] || {};
      }

      delete relays[componentName].pending;
      relays[componentName].code = code;
      relays[componentName].created = Date.now();
...
module.exports.createRelayEvented = function($happn, relaySpec, callback) {
...
module.exports.createDynamicModule = function(type, $happn, connection, relaySpec, callback) {
  ...
  relays[componentName].targetEventApi = targetEventApi;
    ...
    return new Promise(function(resolve, reject) {
      targetEventApi.on(path,
        ...
        function(e, refId) {
          ...
          relays[componentName].subscriptions[refId] = 1;
          resolve();
        }
      );
    });
